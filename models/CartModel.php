...
class CartModel
{
    ...
    {
        $sql = "SELECT * FROM cart_items WHERE cart_id=? AND product_id=? AND size=?";
        $stmt = $this->conn->prepare($sql);
    ...
    {
        $sql = "INSERT INTO cart_items (cart_id, product_id, size, quantity, created_at)
                VALUES(?, ?, ?, ?, NOW())";
        $stmt = $this->conn->prepare($sql);
        return $stmt->execute([$cartId, $productId, $size, $qty]);
    }
